...
description: Create or update project constitution with behavior-first principles. Invoke when the user wants to set up behavior-kit in a project for the first time, or amend the constitution's articles.
---
...
> **Codex note:** invoke this skill as `/bk-constitution` (Codex slash names can't contain dots, so `/bk.constitution` becomes `/bk-constitution`).
...
1. **Worktree decision (REQUIRED — ask first, before anything else).** Check whether the constitution already records a decision:
   ...
   > Do you want to use **git worktrees** for parallel agents? This lets multiple Claude/Cursor/Codex sessions work on different features at the same time without stepping on each other's branches.
   >
